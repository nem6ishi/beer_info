...
# Setup
supabase, logger = setup_script("manual_link_shigakogen")

async def manual_link():
    target_url = "https://untappd.com/b/tamamura-honten-co-shiga-kogen-porter-cask-finished-french-oak-puncheon/6533780"
    target_name_keyword = "%志賀高原ビール　ポーター　カスクフィニッシュ%"
    
    print(f"🔗 Manually linking Shiga Kogen Porter to: {target_url}")
    ...
    for beer in beers:
        ...
        # The user's input had "1t IPA" in parens, likely confusing search.
        # We enforce the correct name.
        updates = {
            'untappd_url': target_url,
            'beer_name_en': "Porter Cask Finished - French Oak Puncheon", 
        ...
        if beer.get('url'):
            try:
                ...
                print("  💾 Updated gemini_data (URL + Name)")
            except Exception as e:
