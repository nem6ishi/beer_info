-- Migration: Add original item name (name) to beer_groups_view select and items jsonb
-- Ensures that when product_type is not 'beer' (e.g. set), the original scraped item name is readily available for UI display.
