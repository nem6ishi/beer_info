-- Migration: Optimize performance with missing indices and fix beer_groups_view to use jsonb
-- Upstream references: scraped_beers(untappd_url), untappd_data(untappd_brewery_url)

-- 1. Create missing indices on foreign-key-like columns for ultra-fast JOINs
...
-- 2. Drop and recreate beer_groups_view using jsonb_agg for robust PostgREST filtering (@> operator)
