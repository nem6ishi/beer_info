...
class TestValidateBreweryMatch(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_result_contains_expected(self):
        self.assertTrue(validate_brewery_match(self._make_element("Stone Brewing Co."), "Stone Brewing"))

    async def test_expected_contains_result(self):
    ...
    async def test_no_expected_brewery_passes(self):
        self.assertTrue(validate_brewery_match(self._make_element("Modern Times"), None))


class TestValidateBeerMatch(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_ordinal_match(self):
    ...
    async def test_what_rough_beast_vs_fresh_hop(self):
    ...
    async def test_what_rough_beast_exact(self):
    ...
    async def test_fresh_hop_variant_self_match(self):
    ...
    async def test_barrel_aged_variant_blocked(self):
    ...
    async def test_nitro_variant_blocked(self):
...
class TestScoreBeerMatch(unittest.IsolatedAsyncioTestCase):

    def _make_element(self, beer_text):
        soup = BeautifulSoup(f'<div class="beer-item"><div class="name"><a href="/b/test/1">{beer_text}</a></div></div>', 'lxml')
        return soup.select_one('.beer-item')

    async def test_exact_match_score_100(self):
        score = score_beer_match(self._make_element("What Rough Beast"), "What Rough Beast")
        self.assertEqual(score, 100)

    async def test_variant_mismatch_score_0(self):
    ...
    async def test_exact_beats_partial(self):
    ...
    async def test_no_expected_beer_returns_100(self):
        score = score_beer_match(self._make_element("Anything"), "")
        self.assertEqual(score, 100)

    async def test_same_variant_with_year_has_positive_score(self):
    ...
    async def test_ddh_abbreviation_matches_expanded(self):
    ...
    async def test_tdh_abbreviation_matches_expanded(self):
...
class TestSearchBreweryBeerScoring(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_scoring_selects_exact_over_variant(self, mock_get_client):
        ...
        result = await search_brewery_beer(
