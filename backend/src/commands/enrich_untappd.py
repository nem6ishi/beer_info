    ...
    try:
        ...
        if not untappd_url and beer.get('url'):
            try:
                ...
                if res.data and len(res.data) > 0 and res.data[0].get('untappd_url'):
        ...
        # Search if no URL found yet or if it's currently a search URL
        if not untappd_url or '/search?' in untappd_url:
            if offline:
